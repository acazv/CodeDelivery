...
<?php 
	...
	if($view == 'store')
	{
		echo "<label><a href='store.php'>Store View</a> > Packages and Features > History > Dependency</label>";
	}
	if($view == 'product')
	{
		echo "<label><a href='product.php'>Product View</a> > Packages and Features > History > Dependency</label>";
	}
?>
...
<table class="table ">
	...
	while($row_dependency=mysqli_fetch_array($result_dependency))
	{
		...
		while($row_featurename=mysqli_fetch_array($result_featurename))
		{
			...
			//button
			echo"<td><button style='background-color: #1E90FF; color:white;'><a href='feature.history.dependency.logon.php?id=".$view."&id1=".$pkey."&id2=".$feature."&id3=".$pkey_dnum."&id4=".$featuredep."&id5=".$pkey_dstatus."&id6=".$pkey_first_dnum."'>Logon</a></button>&nbsp;</td>";
