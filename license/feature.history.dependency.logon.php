...
<?php
	...
	$pkey_first_dnum = (isset($_GET['id6']) ? $_GET['id6'] : null);
...
<?php 
	...
	if($view == 'store'){	
		echo "<label><a href='store.php'>Store View</a> > Packages and Features > History > Dependency > Logons</label>";
	}
	if($view == 'product'){	
		echo "<label><a href='product.php'>Store View</a> > Packages and Features > History > Dependency > Logons</label>";
	}
...
<?php
	...
	<th> Name </th>
	<th> Deploy Status </th>
	<th><button style='background-color: #708090; color:white;'><a href='feature.history.dependency.php?id=".$view."&id1=".$pkey."&id2=".$feature_pkey."&id3=".$pkey_dnum."&id4=".$pkey_dstatus."&id5=".$pkey_first_dnum."'>Back</a></button></th>
	</tr>";

	//display all logons for selected feature dependency
	$sql_productdep_logon = "SELECT * FROM store_productdep_logon WHERE dnum = '$pkey_dnum' AND feature = '$feature_pkey' AND featuredep = '$pkey_featuredep' ORDER BY dnum DESC";
	$result_productdep_logon = mysqli_query($con, $sql_productdep_logon)or die('Could not query database at this time');
	while($row_logon=mysqli_fetch_array($result_productdep_logon))
	{
		...
		//status name
		$sql_dnum_name = "SELECT * FROM store_delivery WHERE snum = '$snum' AND dnum = '$dnum_select'";
